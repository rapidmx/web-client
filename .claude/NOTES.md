...
- **Sidebar sections.** Logic in `apps/shared/mail/useCollapsedSections.ts` (`useSidebarSections`, `sectionChosenOpen`, `read/writeSectionChoices`, `ALL_MAILBOXES_SECTION = "all"`), markup in `components/mail/layout/SidebarSection.tsx`, the unread chip moved out of MailShell into `FolderBadgeChip.tsx`. Storage is `rapidmx:mail-sidebar-collapsed:<userUid>` holding `{sectionId: collapsed}` (booleans only; any read or write failure is swallowed). Default: All mailboxes and the primary mailbox (`primaryMailboxUid()`) open, others collapsed; an explicit choice replaces the default for that section only.
- **The active section is derived open and not persisted**, and its toggle is a no-op with `aria-disabled` and a title, so the section of the open folder can never be hidden. Because the selection defaults to the primary mailbox's Inbox, the primary section is locked open while you are on it. With one mailbox there are no toggles and no badge, and a stored choice is ignored. A folder-load error Alert stays visible when its section is collapsed. Collapsed lists stay in the DOM with `hidden`, so tests query them with `{hidden:true}`. The collapsed badge is `unreadBadgeTotal()` (unread-kind badges only: Drafts/Outbox totals, Sent, Deleted and Junk are excluded).
- **Not collapsible on purpose:** Contacts/Tasks (a mailbox `<select>`, no sections) and the calendar's My calendars groups (their checkboxes drive what is drawn, so hiding them - or defaulting shared mailboxes to hidden - would hide state that is on screen). It would be a small follow-up on the same hook.
