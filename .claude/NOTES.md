...
### 2026-09-25 - why www keeps its own router under @rapidrest/react 2.0.0

`@rapidrest/react` 2.0.0 has a client-side router, but it remounts the whole page subtree on every navigation, has no client-side layout, and remounts on a query-only change, so it cannot keep our persistent `AppChrome` (compose windows, unlock prompt, idle timers) or a folder switch's state. www therefore keeps `apps/shared/navigation/*`, `apps/www/_routedPage.tsx` and `apps/www/_routes.ts` (597 lines plus about 45 lines of focus/announce code in `AppShell`); admin and escrow use the library router (the server turns it on; see the server's NOTES). Delete our router when the library gains a persistent shell, shallow (query-only) navigation, focus/announce/pending, and idle prefetch. `data-full-reload` is the library's `data-router-ignore`.
