...
### 2026-09-26 - www runs on @rapidrest/react 2.1.0's router and app shell; the bespoke router is deleted

`apps/www/_shell.tsx` is the persistent frame (AppChrome + `AppFrameContext` takeover counter), rendered by the server around every page and hydrated with it as one root. It takes the page's props from `WwwRoute.fetchProps()`, so branding, pluginNav and impersonation follow the server on every navigation (one `fetchProps()` request per page change). The rail key is `activeAppOf(route)`. Focus, scroll and the announcement are `useNavigationEffects()` in the shell; the tab title is each page's `title` export (`pageTitle("Calendar")`), and `useUnreadTitle`'s `resetKey` is the pathname. Deleted: `shared/navigation/{AppRouter,routes,routerContext,idle}` and `www/{_routedPage,_routes}` (567 lines) plus the focus/announce/title code in `AppChrome`.

- **Traps:** (1) react-shared's `useBranding()` sets `document.title` when its fetch answers; `AppChrome` puts the page's title back (`pageTitleRef` layout effect + effect, frame only) - the real fix belongs in react-shared. (2) A link that changes only the query on the current route needs `data-router-shallow` (else the library refetches and remounts) and `data-router-prefetch="false"` (else `links: true` warms it on hover); `useNavigate()` makes a same-path destination shallow. (3) The router location includes the request's query on the server, so MailShell reads the selection at first render; `hash` is empty until after hydration. (4) `@rapidrest/react` is a peer dependency (>=2.1.0 <3) and a devDependency; electron-client and the plugins need it resolvable. (5) Admin and escrow pages use `useRouter().navigate` for same-console redirects; cross-app redirects stay `window.location.href`. (6) Adding `_shell.tsx` needs the client build restarted under `yarn dev` (www answers 500 until the manifest has `apps/www/__router`). (7) A server with `router = true` on www needs this web-client; with the old one two routers would fight over clicks.
- **Tests:** `test/apps/routerTestUtils.tsx` (fake RouterApi and mini router: `withTestRouter`, `latestRouter()`), `shell.navigation.test.tsx` runs the real `startRouter` + `_shell` + chrome against a fake fetch, and `wwwPages.test.tsx` keeps every page's `title` and its shell consistent with `activeAppOf`. No `useBlocker` for Compose yet. Not exercised in a browser: plugin pages, `yarn dev` watch mode, real auth, the push WebSocket.
- **Breaking for importers:** `shared/navigation/{AppRouter,routerContext,routes,idle}.js` are gone; `useNavigate` is `shared/navigation/index.js`; `data-full-reload` (never used) is the library's `data-router-ignore`.

### 2026-09-26 - shells read the location through shared/navigation, not the library's useLocation

`@rapidrest/react`'s `useLocation()` is empty outside a router, and plugin pages (`router = false`) are outside one, so a shell that used it lost `?mailboxUid=` on every plugin page (found by booking-plugin's tests against 0.17.0, fixed in 0.17.1). The shells import `useLocation` from `apps/shared/navigation/index.ts`, which uses the router's location when there is one and otherwise the browser's address through `useSyncExternalStore` (empty on the server and while hydrating, so a plugin page hydrates as it was rendered and then follows the address). Any new code that reads the location must use that one. **Lesson:** run the plugins' tests (meet, booking) against a web-client build before releasing it; the web-client suite renders under a `TestRouter` and cannot see this.

### 2026-09-26 - Diagnostics page (`/admin/diagnostics`)

UI half of the admin Diagnostics page (the server half is built separately in `server`; the API contract below is what it was written against). Not committed, no version bump. `apps/admin/diagnostics/index.tsx` is the thin wrapper; the UI is `apps/shared/components/admin/diagnostics/` (`DiagnosticsManager` holds the tabs, the Versions/Runtime/plugins reads, the metrics polling and the log stream); one `AdminSection` id (`diagnostics`) and one `NAV_ITEMS` entry (`HiOutlineCpuChip`, after Plugins) in `AdminShell.tsx`, the only shared file touched. Tests are `test/apps/admin/diagnostics/*` (unit files per module, `DiagnosticsManager.test.tsx` for the page behaviour, `index.ssr.test.tsx`).

- **Contract** (`diagnosticsApi.ts`, typed `apiFetch` calls to `/admin/diagnostics/versions|runtime|metrics`, all GET, trusted-admin + elevation): namespace-scoped RBAC only, so a runtime node is just `{ name, internalIP?, podCount }` and there are no per-node or kubelet numbers. `metrics.host` is the node the server pod runs on as its own container sees it (CPU %, memory, load, disks), `kubernetes.namespace` has pod CPU/memory only when `podMetricsAvailable` (metrics-server; the reason is shown as a hint, not an error), and `pvcs[].usedBytes` exists only for volumes with `mountedByServer` (`sharesNodeDisk` volumes are shown but never flagged High/Critical).
- **Elevation**: `describeError()` in `format.ts` maps `api-104` to a "confirm your identity" message (`isElevationRequired`, as `PluginsManager` does), any other 401/403 to "not authorised". A refusal on the metrics poll pauses polling (Resume retries) instead of asking every 5 s.
- **Polling** (`useMetricsPolling`): `METRICS_POLL_INTERVAL_MS` = 5000, the last `METRICS_HISTORY_SIZE` = 60 samples, only while the System tab is open (the history survives leaving the tab, the polling does not), never two requests in flight, none while `document.hidden` (one at once on `visibilitychange` to visible), stops on unmount. Versions/Runtime/plugins are read on open and on Refresh only.
- **Charts** follow the dataviz skill: one hue (the `primary` token), 2 px line with a 10% wash, the value always written as a number beside it, a text alternative with latest/lowest/highest, meters that say High (>80%) and Critical (>90%) in words with an icon. No chart library.
- **Logs** (`logClient.ts`, a local copy of the `PushClient` pattern - react-shared untouched): `<origin>/api/admin/logs` from `apiUrl()` with http->ws; the first `{"id":0,"type":"SUBSCRIBED"}` frame flips the status to live and is not a line; close code 1002 is a final error (no reconnect; Start retries); other closes back off 1 s doubling to 30 s with jitter. Non-JSON frames are shown raw (`RAW`), missing fields tolerated, ANSI colour codes stripped from level and message. `LogStore` keeps 5,000 lines (counting dropped ones) and, independently, a capture of up to 50,000 (keeps the start, counts what it missed). Redraws are batched every 100 ms. The socket opens the first time the Logs tab is opened and lives with the page, so a capture survives switching tabs.
- **Downloads** go through an injectable `SaveFile` (`saveTextFile` = Blob + object URL + temporary `<a download>`, URL revoked on the next tick); `.log` is `<timestamp> <LEVEL> <message>` one line per entry (line breaks in a message folded to ↵), `.ndjson` one JSON per line (a frame that was not JSON becomes `{"raw":"..."}`). The diagnostics report leaves out plugin settings (they can hold secrets) and logs.
- **Checks**: `yarn lint`, `yarn tsc --noEmit -p tsconfig.json` clean; the diagnostics files are at 100% statements/branches/functions/lines. `tsc -p tsconfig.test.json` already has unrelated errors in older test files (not from this work).
