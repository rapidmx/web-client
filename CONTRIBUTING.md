...
- The **Project Info** containing the version of this package, which consumer you hit it in
  (`rapidmx/server`'s SSR pages or `electron-client`), your browser/React version, and your
  Node.js version
...
```
Steps to Reproduce:
1. `yarn build` this package and consume it from a fresh Vite app
2. Render `SettingsReadReceiptsPage` with a `mailboxUid` that has no matching mailbox
3. Observe the browser console

Repro Rate: 3/3

Expected:
The page shows a "no mailbox selected" state

Actual:
A TypeError is thrown ("Cannot read properties of undefined")
// Insert full stack trace / browser console output

Severity: HIGH

Project Info:
@rapidmx/web-client: v0.1.0
...
```
As a developer I would like `AppShell` to expose a `theme` prop, so consumers outside
`rapidmx/server`'s own SSR pages can opt into dark mode without duplicating its CSS variables.
```
