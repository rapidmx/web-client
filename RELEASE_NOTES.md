...
- **Guests in the calendar event dialog use the same lookup and chips as the compose window.** Typing a name in Guests (the quick popover, the full form and a guest's Add guests request) suggests matching contacts, mailboxes and distribution lists, and a chosen one becomes a chip with its name and address, saved as a guest with both. Addresses, `Name <address>`, several pasted at once, commas, semicolons, Enter and leaving the field all add guests. Something that is not an address stays as a flagged chip until you remove or fix it, with no error while you type. Guests already invited and the organizer are not added twice. A distribution list is added as one guest with the list's own address.

- **Search works across all mailboxes.** The search box in the All mailboxes view is on: it searches every mailbox you can read (your own, shared and delegated), up to 25 at once, and merges the results by relevance, each row naming its mailbox. From one mailbox's folder, "Search all mailboxes" widens the search and "Search this mailbox only" narrows it back. A mailbox that cannot be searched (no access, too slow, an error) is named in a "Some results may be missing" note while the others' results stay. Encrypted mail is searched in each mailbox you have unlocked: through the local index where this device has one (it is kept for the open mailbox), through the server otherwise, with a note saying which, and a button to unlock your own locked mailboxes. Results can be ticked, archived, deleted and marked from the selection bar or the keyboard, each message in its own mailbox's folders; Move to and Apply label are offered only for messages of one mailbox. "Search all mail" reads "Search older encrypted mail" when the search covers several mailboxes.

- **The Mail sidebar's sections collapse.** "All mailboxes" and each mailbox are disclosure headings (a turning chevron, Enter or Space to toggle) that start with All mailboxes and your primary mailbox open and every other mailbox collapsed, so a newly shared mailbox arrives collapsed; any section can be collapsed, the primary mailbox and the one holding the open folder included. Your choices are remembered per browser, choosing a folder in (or linking to) a collapsed section opens it, a collapsed section shows its unread total across its Inbox, Archive and your own folders on its heading (Drafts, Outbox, Sent Items, Deleted Items and Junk Email are left out), and the phone's folders drawer works the same. A lone mailbox's sidebar is unchanged.
...
- **The New event popover has Event, Task and Appointment schedule tabs.** Task creates a task in the chosen mailbox's Tasks folder with a due date (all day or a time) and a task list, and "More options" adds notes, priority, a reminder and My Day; it shows a "Task added" notification and closes. Appointment schedule (shown when the booking plugin is running) publishes a bookable schedule with a name, duration, weekdays and hours, a buffer and video, phone or other locations, then shows its public link with a Copy button, and "More options" opens the plugin's new booking link page.
- **Events have a rich-text description** (bold, italic, underline, numbered and bulleted lists, links, remove formatting; paste keeps only text) in the event card and as an "Add description" row in the quick popover, shown read-only in the event details and on the invitation card.
- **Events have a visibility** (Default, Public, Private, Confidential) next to Busy status; a private or confidential event on a shared calendar shows to read-only readers as a plain "Busy" block, and the calendar refetches such an event when a live update announces it.
- **Events have guest permissions** (Modify event, Invite others, See guest list); with the list hidden an invited reader sees only themselves, and a guest whose event allows it gets "Request a change" / "Add guests" in the event details and on the invitation card, which asks the organizer (the organizer's card says "X requested a change" or "X's change was applied").
- **A Find a time tab in the event card** shows you and your guests' day side by side with the proposed time highlighted and up to five suggested times; people whose availability cannot be seen are shown as such, never as free.
- **A Free/busy visibility setting on the Profile page** chooses who may see your free and busy times: your domain (default), only people you share your calendar with, nobody, or everyone on this server.

- **An existing calendar event opens to its details first.** The dialog shows when, location, the video call, the organizer, the guests and their responses, the calendar, busy status and reminder. The organizer (or the owner of a personal event) gets Modify and Delete, and Modify opens the edit card; an invited reader gets Accept, Tentative and Decline (their current answer is marked) and can remove the event from their calendar, but cannot change it. X or Escape in the edit card returns to the details and discards the edits.
- **New events are made in a quick-create popover next to what you clicked**, as in Google Calendar (a bottom sheet on a phone): title, when (date and times expand in place, with All day, Time zone and repeat), guests as chips, video conferencing, location and the calendar, with "More options" opening the full card without losing what you typed. The full card has separate date and time fields, a time zone choice, a recurrence menu (Custom opens the recurrence editor), a notification with a unit, guests with roles, and rooms and equipment. Clicking an empty part of a day in the month view starts an all-day event, and changing an event's start now moves its end with it.

- **A meeting invitation in a message can be answered from the message, and from the message list, as in Outlook.** The reading pane (and each expanded message of a conversation) shows an invitation card for a message with an `.ics`: Accept, Tentative and Decline (the answer given is marked and can be changed), Propose new time, "Add to calendar" for a published event, "Remove from calendar" for a cancellation and an "Open in Calendar" link; the server puts the meeting on the calendar (or leaves it off, for a decline) and mails the organizer the reply. A "Conflicts with ..." line names your own events that overlap. A reply to your own invitation reads "Jean-Philippe Steinmetz tentatively accepted." instead of showing a nameless attachment, a proposed new time shows an "Accept proposal" button, and the `.ics` is no longer listed as an attachment while the card is shown. Under a meeting request in the message list (a conversation's row, a plain row and an expanded conversation's rows) there is an RSVP chip - when it starts, "No conflicts" / "Conflicts with N events" or your answer, and an RSVP button that opens a day view of your schedule with the meeting highlighted and Accept, Decline, Tentative ("?") and a "..." menu with Propose new time and Open in calendar. Pressing the chip neither opens the row nor starts a swipe. Test all of it.
...
- **Swipe the calendar on a phone.** Swiping left shows the next month, week or day and swiping right shows the previous, using the same navigation as the Previous and Next buttons; it leaves vertical scrolling and taps alone and is off while an event is being dragged or edited.
...
- **Test coverage kept in sync with an upcoming `@rapidmx/react-shared` CSRF fix** (not yet a dependency bump in this repo): `stopImpersonating()`'s HTTP method is changing from GET to POST upstream — a state-changing GET is exploitable via a bare navigation, bypassing CSRF defenses entirely. This repo calls it only through `react-shared`, so no source change was needed, but `MailShell`/`AppShell`'s mocked-fetch tests were updated to expect POST ahead of that dependency bump.
...
- **Unchecking video conferencing on a single occurrence of a recurring series no longer cancels the whole series' shared meeting.** Editing just one occurrence ("This event only") detaches it into its own standalone event that was never meant to carry the series' meeting - but the checkbox's own save logic compared against stale state left over from before the detach, so unchecking it there silently cancelled the meeting for every *other* occurrence still using it too, with no warning anywhere. That occurrence's own detach is unaffected; the series' meeting is now left alone unless it is genuinely turned off on the series itself.
...
- **A fully-scrolled large mailbox no longer keeps loading every message into memory forever.** Infinite scroll had no ceiling on total rows, only on a streak of empty pages - scrolling to the end of a big folder or search accumulated every page ever fetched as live state. The message and conversation lists now stop growing at 500 rows and show "Showing the most recent 500 ... - refine your search or filters to see more" instead of continuing to fetch further pages - unless the folder or search was already fully loaded, in which case that banner no longer appears just because its true size happened to land at or past 500.
- **"Add video conferencing" fails clearly on a server without the plugin installed**, instead of with a raw, confusing 404: the event still saves, and the checkbox's inline error now says plainly that video conferencing isn't available on this server.
- **Auto-encryption no longer gets skipped just because the same recipient was typed with different casing across To/Cc/Bcc** (a plausible outcome of autocomplete in one field and a pasted address in another). The two are now recognized as one recipient, matching how the encryption-tier decision already compared addresses.
...
- **Domain aliases in the admin console.** "New domain" gains an **Alias of** dropdown listing this server's own non-alias domains - picking one makes the new domain a pure alias, so it receives (and
  may send as) mail for the same addresses as the domain it aliases with no mailboxes of its own. A domain's detail page shows its alias target (if any) and lets it be set or changed after the fact,
  and the domains list shows an "Alias of" column. Needs `@rapidmx/react-shared`'s `admin/domainsApi.js` `aliasOf` field and an `@rapidmx/restapi` with the domain-alias feature.
- **Add video conferencing to a meeting.** A new **Add video conferencing** checkbox under Location on the event form. Ticking it and saving mints a video meeting for the event's attendees (everyone but you) through `@rapidmx/meet-plugin`, sets the event's location to "Video call — link in this invitation" and links the meeting to the event; each attendee then gets their own personal join link in their own copy of the invitation, and you get a **Join video call** button on the event itself. Unticking it cancels the meeting, drops the link and clears that location again - unless you have since typed your own, which is kept. Changing attendees afterwards does not reissue links (the meeting's invitee list is fixed when it is created, as the note under the checkbox says): turn it off and back on to reissue them for the current list. If the meeting service fails, the event is still saved - the error appears next to the checkbox and Save retries it. Needs `@rapidmx/react-shared`'s `videoconf/videoMeetingsApi.js` and a server with the video conferencing plugin installed.
- **Meeting reminder pop-ups.** An event's "Remind me N minutes before" now does something on the web client: when `@rapidmx/restapi`'s reminder job fires, a sticky pop-up appears - wherever you are in the app, not just on the Calendar page - with **Dismiss** (just closes it) and **Snooze** (closes it and shows it again in five minutes, or sooner if the meeting starts first). Uses the same push connection Mail already keeps open, so it needs no extra socket or polling. When the event's location is itself a link (a video call URL, for instance), the pop-up leads with a **Join Meeting** button that opens it in a new tab and leaves the pop-up up for Dismiss/Snooze afterward. Needs an `@rapidmx/restapi` whose reminder event carries `location`.
