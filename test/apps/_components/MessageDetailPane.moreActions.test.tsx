...
describe("Report junk on the card", () => {
    ...
    it("moves the message to its mailbox's Junk Email folder, tells the caller, the badges and the search index, and the reader", async () => {
        ...
        // Nothing but the move: the server has no junk-report feedback to call.
        expect(fetchMock.mock.calls.map(([url]) => url)).toEqual(["/api/mail/messages/m1"]);
    });
    ...
    it("waits while it is working, so a second click cannot report it twice", async () => {
        ...
        const fetchMock = serve((url, init) => (url === "/api/mail/messages/m1" && method(init) === "PUT" ? new Promise<Response>((resolve) => (answer = resolve)) : undefined));
        const user = userEvent.setup();
        ...
        await user.click(screen.getByRole("button", { name: "Report junk" }));
        expect(bodiesOf(fetchMock, "PUT", "/api/mail/messages/m1")).toHaveLength(1);
        answer(jsonResponse(200, next(message() as Record<string, any>, { folderUid: "f-junk" })));
        await waitFor(() => expect(screen.getByRole("button", { name: "Report junk" })).toBeEnabled());
    });

    it("is disabled, with the reason as its tooltip, for a message already in Junk Email", () => {
        serve();
        render(<MessageDetailPane message={message({ folderUid: "f-junk" })} attachments={[]} folders={FOLDERS as never} />);
        const button = screen.getByRole("button", { name: "Report junk" });
        expect(button).toBeDisabled();
        expect(button).toHaveAttribute("title", "Already in Junk Email");
    });

    it("is disabled in a mailbox shared with the reader view-only, and enabled in one they may update", async () => {
        ...
        const access = { canRead: true, canCreate: false, canUpdate: false, canDelete: false, canManage: false };
        serve((url) => (url === "/api/mail/mailboxes/mb1/access/me" ? jsonResponse(200, access) : undefined));
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} />);
...
describe("Report > Report phishing", () => {
    it("files the message in Junk Email like Report junk, and says plainly that nobody was told", async () => {
        const original = message() as Record<string, any>;
        const moved = next(original, { folderUid: "f-junk" });
        const fetchMock = serve(putTo("m1", () => moved));
        const onMoved = vi.fn();
        ...
        expect(popups()).toEqual([
            expect.objectContaining({
                ...
                message: expect.stringContaining("This report was not sent to anyone"),
            }),
        ]);
    });

    it("has Report junk in the same submenu, doing what the card's button does, and names a message with no subject", async () => {
        const original = message({ subject: "" }) as Record<string, any>;
        const moved = next(original, { folderUid: "f-junk" });
        const fetchMock = serve(putTo("m1", () => moved));
        const onMoved = vi.fn();
        const user = userEvent.setup();
        ...
        await waitFor(() => expect(onMoved).toHaveBeenCalledWith(moved));
        expect(bodiesOf(fetchMock, "PUT", "/api/mail/messages/m1")).toEqual([{ uid: "m1", version: 3, folderUid: "f-junk" }]);
        expect(popups()).toEqual([expect.objectContaining({ title: "Reported as junk", message: "“(no subject)” was moved to Junk Email." })]);
    });

    it("reports a refusal as such", async () => {
        serve((url, init) => (url === "/api/mail/messages/m1" && method(init) === "PUT" ? jsonResponse(403, { message: "no" }) : undefined));
        const user = userEvent.setup();
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} />);
        await choose(user, ["Report"], /^Report phishing/);
...
describe("Block > Block <sender>", () => {
    ...
    it("makes a rule for the sender's addresses (the one it is filed under and the From header's), moves this message to Junk, and offers Undo and View rules", async () => {
        const original = message() as Record<string, any>;
        const moved = next(original, { folderUid: "f-junk" });
        const fetchMock = serve(raw, rules([]), createRule, putTo("m1", () => moved));
        const onMoved = vi.fn();
        const user = userEvent.setup();
        render(<MessageDetailPane message={original as never} attachments={[]} folders={FOLDERS as never} onMoved={onMoved} />);
    ...
    it("makes no second rule when the sender is already blocked, says so, and still moves this message", async () => {
        const original = message() as Record<string, any>;
        const fetchMock = serve(raw, rules([junkRule()]), putTo("m1", () => next(original, { folderUid: "f-junk" })));
        const onMoved = vi.fn();
        const user = userEvent.setup();
        render(<MessageDetailPane message={original as never} attachments={[]} folders={FOLDERS as never} onMoved={onMoved} />);
        ...
        expect(bodiesOf(fetchMock, "POST", "/api/mail/mail-filter-rules")).toEqual([]);
        expect(popups()).toEqual([
            expect.objectContaining({ kind: "info", title: `${SENDER} is already blocked` }),
        ]);
    ...
    it("leaves a message that is already in Junk where it is, and says so", async () => {
        const fetchMock = serve(raw, rules([]), createRule);
        const onMoved = vi.fn();
        const user = userEvent.setup();
    ...
    it("blocks the one address when the From header is the one the message is filed under", async () => {
        const fetchMock = serve(
            (url) => (url === "/api/mail/messages/m1/raw" ? new Response("From: SENDER@example.com\r\n\r\nx") : undefined),
            rules([]),
            createRule,
            putTo("m1", () => next(message() as Record<string, any>, { folderUid: "f-junk" })),
        );
    ...
    it("moves nothing and says so when the rule could not be made", async () => {
        serve(raw, rules([]), (url, init) => (url === "/api/mail/mail-filter-rules" && method(init) === "POST" ? jsonResponse(400, { message: "bad rule" }) : undefined));
        const onMoved = vi.fn();
        const user = userEvent.setup();
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} onMoved={onMoved} />);
        await choose(user, ["Block"], `Block ${SENDER}`);
    ...
    it("says the sender is blocked but the message could not be moved, when the move fails", async () => {
        serve(raw, rules([]), createRule, (url, init) => (url === "/api/mail/messages/m1" && method(init) === "PUT" ? jsonResponse(500, { message: "boom" }) : undefined));
        const onMoved = vi.fn();
        const user = userEvent.setup();
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} onMoved={onMoved} />);
        await choose(user, ["Block"], `Block ${SENDER}`);
...
describe("Block > Never block <sender>", () => {
    ...
    it("takes away the block on the sender, and says so", async () => {
        const fetchMock = serve(raw, rules([rule("block", "f-junk", "Block sender@example.com")]), createRule, (url, init) =>
    ...
    it("makes nothing when the sender is already never blocked", async () => {
        const fetchMock = serve(raw, rules([rule("keep", "f1", "Never block sender@example.com")]));
        const user = userEvent.setup();
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} />);
        await choose(user, ["Block"], `Never block ${SENDER}`);
        await waitFor(() => expect(popups()).toHaveLength(1));
    ...
    it("says what could not be done", async () => {
        serve(raw, (url) => (url.startsWith("/api/mail/mail-filter-rules?") ? jsonResponse(500, { message: "boom" }) : undefined));
        const user = userEvent.setup();
        render(<MessageDetailPane message={message()} attachments={[]} folders={FOLDERS as never} />);
        await choose(user, ["Block"], `Never block ${SENDER}`);
