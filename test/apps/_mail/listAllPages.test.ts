...
describe("listAllPages", () => {
    it("stops at the first short page", async () => {
        ...
        expect(await listAllPages(fetchPage, 2, 10)).toEqual([0, 1, 10, 11, 99]);
    ...
    it("stops after an empty page when the total is an exact multiple of the page size", async () => {
        ...
        expect(await listAllPages(fetchPage, 2, 10)).toEqual(["a", "b"]);
        expect(fetchPage).toHaveBeenCalledTimes(2);
    });

    it("gives up after maxPages when every page is full", async () => {
        const fetchPage = vi.fn(async () => [1, 2]);
        expect(await listAllPages(fetchPage, 2, 3)).toEqual([1, 2, 1, 2, 1, 2]);
