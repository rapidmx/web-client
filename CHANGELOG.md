...
### Added
- Added a Diagnostics page to the admin console with server, plugin and container versions, the Kubernetes runtime, live node, namespace and volume usage, and a live log viewer
- Added log capture that records every line of the server log and downloads it, or the buffer, as .log or .ndjson
- Added a downloadable diagnostics report of the versions, runtime and a metrics sample for support

### Changed
- Test the Diagnostics page, its polling, log socket, capture and downloads
- Note the page in the release notes and NOTES

## [0.17.2] - 2026-09-26

### Changed
- Join the parts of the browser's address, so a test's stand-in for window.location with no query does not make the location NaN
...
[Unreleased]: https://github.com/rapidmx/web-client/compare/v0.18.0...HEAD
